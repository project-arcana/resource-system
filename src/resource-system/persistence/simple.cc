...
bool res::persistence::SimplePersistentStore::load()
{
    ...
    LOG("using persistency cache (%s invocs, %s contents, %.1f MB)", invocs.size(), contents.size(), content_data_size / 1024. / 1024.);
    return true;
}
...
bool res::persistence::SimplePersistentStore::save()
{
    ...
    LOG("written %s new cached contents to '%s' (%.1f MB)", new_contents.size(), content_filename(), new_content_total_size / 1024. / 1024.);
